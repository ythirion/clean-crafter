...
:green_circle: We need to adapt a bit the `getLoggedUser` from our seam
...
:large_blue_circle: We clean up a little the code as well
...
:green_circle: Let's cover the `happy path` now
```java
@Test
...
:large_blue_circle: [Test Data Builders](http://www.natpryce.com/articles/000714.html) can help you when writing / maintaing unit tests:
